...
<c:forEach var="AllSpecies" items="${AllSpecies}">
 <button  onClick="location.href='selectforumspecies?fiid=${AllSpecies.fsid}&currpage=0';">${AllSpecies.species}</button>
  ...
  <button id="create-forum" onClick="location.href='gocreateeditor?forumid=${forumid}';">我要發文</button>
</div>
   ...
   <c:forEach var="AllForum" items="${AllForum}">	
	 ...
	 <a href="goforumdetail?forumid=${AllForum.forum.fid}" class="list-group-item">
      ...
      ${AllForum.forumdetail.content}
		...
		<ul class="pagination">
		  <li class="page-item"><a class="page-link" href="goforum?currpage=0&fiid=${copyfid}">&laquo;</a></li>
		  <c:forEach begin="1" end="${totalpage}" var="page">
		  <c:if test="${page==currpage+1}">		 
		  <li class="page-item active" class="page-item"><a class="page-link" href="goforum?currpage=${page-1}&fiid=${copyfid}">${page}</a></li>		 
		 </c:if>
		  <c:if test="${page!=currpage+1}">		 
		  <li class="page-item " class="page-item"><a class="page-link" href="goforum?currpage=${page-1}&fiid=${copyfid}">${page}</a></li>		
		  </c:if>
		  </c:forEach>
		  <li class="page-item"><a class="page-link" href="goforum?currpage=${totalpage}&fiid=${copyfid}">&raquo;</a></li>
		</ul>
		</c:if>
