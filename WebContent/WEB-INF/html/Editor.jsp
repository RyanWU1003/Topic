...
<form action="fileuploadcontroller?forumid=${id}&fdid=${fdid.fdid}&${_csrf.parameterName}=${_csrf.token}" method="post" enctype="multipart/form-data">
 <dxa:csrf-token/>
       <c:if test="${fdid.fl==1}">
...
<c:when test="${fdid.content==null}">
<form action="fileuploadcontroller?forumid=${id}&fdid=0&${_csrf.parameterName}=${_csrf.token}" method="post" enctype="multipart/form-data">
      <input readonly="readonly" type="text" name="title" value="${title}" size="35" />
        <textarea name="content" id="editor"  >
...
<button><a href="goforumdetail?forumid=${id}" class="show-update">取消編輯</a> </button>
